"use strict";

var _models = require("../models");

var _models2 = _interopRequireDefault(_models);

var _constants = require("../helpers/constants");

var _moment = require("moment");

var _moment2 = _interopRequireDefault(_moment);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

var path = require("path");
const { Template } = require("@walletpass/pass-js");


const template = new Template("generic", {
    passTypeIdentifier: _constants.PASS_TYPE_IDENTIFIER,
    teamIdentifier: _constants.TEAM_IDENTIFIER,
    backgroundColor: "#345C8C",
    sharingProhibited: true,
    organizationName: "Saguaro Lab",
    labelColor: "#74cefb",
    foregroundColor: "#fff"
});

template.setCertificate(`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`);

template.setPrivateKey(`-----BEGIN PRIVATE KEY-----

v02aQuEMecxQCj9dhlzW
-----END PRIVATE KEY-----`);

exports.apple_pass = async (req, res, next) => {
    try {
        let year = (0, _moment2.default)().add(10, 'years').format('YYYY');
        let month = (0, _moment2.default)().add(10, 'months').format('MM');
        let days = (0, _moment2.default)().add(10, 'days').format('DD');

        let { qr_code } = req.body;

        let find_member = await _models2.default.Member.findOne({
            where: {
                qr_code: qr_code
            }
        });

        if (find_member.qr_code === null) {
            return res.status(200).json({
                status: "failed",
                payload: null,
                message: "invalid Qr"
            });
        }

        console.log((0, _moment2.default)(find_member.birth_date).format('MM/DD/YYYY'));

        await template.images.add("icon", path.join(__dirname, "../images", "iconNew.png"));
        await template.images.add("logo", path.join(__dirname, "../images", "logoNew.png"));

        const pass = template.createPass({
            serialNumber: "123456",
            description: "Bloom Lab Pass"
        });

        pass.primaryFields.add({ key: "new", label: "", value: "BLOOM LABS" });
        pass.secondaryFields.add({ key: "name", label: "NAME", value: find_member.full_name });
        pass.auxiliaryFields.add({ key: "dat", label: "DATE OF BIRTH", value: find_member.birth_date !== null ? (0, _moment2.default)(find_member.birth_date).format('MM/DD/YYYY') : "" });
        pass.auxiliaryFields.add({ key: "info", label: "INFORMATION", value: "Your Personal Qr Code" });
        pass.expirationDate = new Date(parseInt(year), parseInt(month), parseInt(days), 10, 0);

        pass.barcodes = [{
            message: "6A072F8",
            format: 'PKBarcodeFormatQR',
            messageEncoding: 'iso-8859-1'
        }];

        const body = await pass.asBuffer();
        res.type('application/vnd.apple.pkpass');
        res.send(body);
    } catch (error) {
        console.log("Error at apple pass ==> " + error);
        res.status(500).json({
            status: "failed",
            payload: {},
            message: "Error while apple pass"
        });
    }
};