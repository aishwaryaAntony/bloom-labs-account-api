...
exports.fetch_all_lab_locations = async (req, res, next) => {
    try {
        ...
        let fetchLabLocations = await _models2.default.LabLocation.findAll({
            limit: limit,
            offset: offset,
            order: [['id', 'ASC']]
        });
...
exports.create_lab_location = async (req, res, next) => {
    try {
        ...
        let fetchLabs = await _models2.default.Lab.findOne({
            where: {
                code: lab_code
            }
        });
        ...
        let new_lab_location = await _models2.default.LabLocation.create({
            lab_id: fetchLabs.id,
            lab_code,
            code,
            name,
...
exports.update_lab_location = async (req, res, next) => {
    try {
        ...
        await _models2.default.LabLocation.update({
            lab_id: fetchLabs.id,
            lab_code,
            code,
            name,
        ...
        let updatedLabLocation = await _models2.default.LabLocation.findOne({
            where: {
                id: id
            }
        });
