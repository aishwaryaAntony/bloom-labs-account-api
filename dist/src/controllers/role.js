...
exports.fetch_all_roles = async (req, res, next) => {
    try {
        ...
        let fetchRoles = await _models2.default.Role.findAll({
            limit: limit,
            offset: offset,
            order: [['id', 'ASC']]
        });
...
exports.create_role = async (req, res, next) => {
    try {
        ...
        let fetchRole = await _models2.default.Role.findOne({
            where: {
                code: code
            }
        });
        ...
        let newRole = await _models2.default.Role.create({
            code,
            name,
            permission,
            status
...
exports.update_role = async (req, res, next) => {
    try {
        ...
        fetchRole = await _models2.default.Role.findOne({
            where: {
                id: fetchRole.id
            }
        });
