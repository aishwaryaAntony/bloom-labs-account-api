...
router.post('/verify-phone', async (req, res, next) => {
	try {
		...
		let otp = _constants.NODE_ENV === "development" ? "000000" : await (0, _accounts.createVerificationCode)();
		console.log(`otp=>${otp}`);
...
router.post('/add-pages', async (req, res, next) => {
	try {
		...
		let permissionSetObj = Object.assign({}, findRole.permission, pages);
...
router.get('/image/:type/:imagename', async (req, res, next) => {
	try {
		...
		let findBucket = _constants.S3_BUCKETS.find(x => x.code === type);
		...
		let data = await (0, _attachments.getImage)(params);
