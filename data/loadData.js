					...
					if (row === lastRow) {
						if (!isRejected === true) {
							for (let user of userArray) {
								...
								let hashed_email = email !== null ? await crypto.hash_from_string(email) : null;
...
const loadAppScreens = () => {
	return new Promise(async (resolve, reject) => {
		let appScreens = [
			{ code: "P-001", name: "Location" },
			{ code: "P-002", name: "Location Test Type" },
			{ code: "P-003", name: "Roles and Permission" },
			{ code: "P-004", name: "Test Type" },
			{ code: "P-005", name: "User and Access" },
			{ code: "P-006", name: "Dashboard" },
			{ code: "P-007", name: "Upload Test Results" },
			{ code: "P-008", name: "Test Results" },
			{ code: "P-009", name: "Patients" },
			{ code: "P-010", name: "Compliance Reporting" },
			{ code: "P-011", name: "Test Uploads" },
			{ code: "P-012", name: "Provide Feedback" },
			{ code: "P-013", name: "Application Screens" },
			{ code: "P-014", name: "Scan Patients" },
		]
		try {
			for (let screen of appScreens) {
				const { name, code } = screen;
				let findScreenPermission = await db.ScreenPermission.findOne({
					where: {
						code: code
					}
				});
				if (findScreenPermission === null) {
					await db.ScreenPermission.create({
						name,
						code,
						status: "ACTIVE"
					});
