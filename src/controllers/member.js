...
import { findSessionAccount, createQrCode, fetchIfUserExist } from "../helpers/accounts";
import crypto from "../helpers/crypto";
const bcrypt = require('bcryptjs');
import { uploadDocument, uploadBase64Image, deleteDocument } from "../helpers/attachments"
import { S3_INSURANCE_BUCKET_NAME, S3_USER_BUCKET_NAME } from "../helpers/constants";
...
exports.fetch_members = async (req, res, next) => {
	try {
		...
		if (page !== undefined && page !== null) {
			...
			fetchMembers = await db.Member.findAll({
				limit: limit,
				offset: offset,
				order: [['id', 'DESC']],
			});
...
exports.fetch_account_member_by_token = async (req, res, next) => {
	try {
		let { token } = req.params;
		let fetchMember = await db.Member.findOne({
			where: {
		...
		let fetchAccountMembers = await db.Member.findAll({
			where: {
				account_id: fetchMember.account_id
			},
			include: [
				{
					model: db.MemberInsurance,
		...
		res.status(200).json({
			status: 'success',
			payload: fetchAccountMembers,
			message: 'Account Members Fetched successfully'
		});
	} catch (error) {
		console.log("Error at fetching account member method- GET / :token " + error);
		res.status(500).json({
			status: 'failed',
			payload: null,
			message: 'Error while fetching account member'
		});
	}
}
...
exports.create_new_member = async (req, res, next) => {
	try {
		...
		let findLoggedInUserAccount = await findSessionAccount(req.userData);
		...
		if (req.files.signature_file !== undefined && req.files.signature_file !== null && req.files.signature_file !== "" && req.files.signature_file.length > 0) {
			signature_image_name = await uploadDocument(req.files.signature_file[0], S3_USER_BUCKET_NAME);
		}
...
exports.create_member = async (req, res, next) => {
	try {
		...
		let {
			...
			birth_date, race, ethnicity, driver_license_number, passport_number, ssn, address_line1, address_line2,
			city, state, country, zipcode, status, role, insurance_provider, policy_number, policy_group_number, street_address_line1,
			street_address_line2, provider_phone_number, insurance_state, insurance_city, insurance_country, insurance_zip_code, primary_account_id, signature_image
		} = req.body;
		...
		*/
		...
		// req.body.lab_code = findLoggedInUserAccount.accountLab.code;
		req.body.lab_code = findLoggedInUserAccount.accountLab !== null ? findLoggedInUserAccount.accountLab.code : 'SBD';
		...
		// for lab executive
		if(findLoggedInUserAccount.accountLab === null){
			findLab = await db.Lab.findOne({
				where: {
					code: req.body.lab_code
					// id: findLoggedInUserAccount.lab_ref
				}
			});
		}
		...
		if (primary_account_id !== null && primary_account_id !== "null") {
			...
			let fetchUser = await fetchIfUserExist(req.body);
			...
			let hashedCountryCode = country_code !== null && country_code !== "null" ? await crypto.hash_from_string(country_code) : null;
			let hashedPhoneNumber = phone_number !== null && phone_number !== "null" ? await crypto.hash_from_string(phone_number) : null;
			let hashedEmail = email !== null && email !== "null" ? await crypto.hash_from_string(email) : null;
...
exports.update_member_by_id = async (req, res, next) => {
	try {
		...
		// 		await deleteDocument(fetchMember.signature_image, S3_USER_BUCKET_NAME);
		// 	}

		// 	signature_image_name = await uploadDocument(req.files.signature_file[0], S3_USER_BUCKET_NAME);
		// }

		if (!!signature_image && signature_image !== "null") {
			if (fetchMember !== null && fetchMember.signature_image !== null) {
				await deleteDocument(fetchMember.signature_image, S3_USER_BUCKET_NAME);
			}
			signature_image_name = await uploadBase64Image(signature_image, S3_USER_BUCKET_NAME);
		...
		if (req.files.id_image_file !== undefined && req.files.id_image_file !== null && req.files.id_image_file !== "" && req.files.id_image_file.length > 0) {
			if (fetchMember !== null && fetchMember.id_card_image !== null) {
				await deleteDocument(fetchMember.id_card_image, S3_USER_BUCKET_NAME);
			}

			id_image_name = await uploadDocument(req.files.id_image_file[0], S3_USER_BUCKET_NAME);
		}

		newMember = await db.Member.update({
		...
		if ((!!insurance_provider && insurance_provider !== 'null' && insurance_provider !== 'undefined') && ( !!policy_number && policy_number !== 'null' && policy_number !== 'undefined')) {
			...
			if (req.files.insurance_front_file !== undefined && req.files.insurance_front_file !== null && req.files.insurance_front_file !== "" && req.files.insurance_front_file.length > 0) {
				if (findMemberInsurance !== null && findMemberInsurance.front_insurance_card_image !== null) {
					await deleteDocument(findMemberInsurance.front_insurance_card_image, S3_INSURANCE_BUCKET_NAME);
				}
			...
			if (req.files.insurance_back_file !== undefined && req.files.insurance_back_file !== null && req.files.insurance_back_file !== "" && req.files.insurance_back_file.length > 0) {
				if (findMemberInsurance !== null && findMemberInsurance.back_insurance_card_image !== null) {
					await deleteDocument(findMemberInsurance.back_insurance_card_image, S3_INSURANCE_BUCKET_NAME);
				}
			...
			if (findMemberInsurance === null) {
				await db.MemberInsurance.create({
					member_id: fetchMember.id,
					insurance_provider: !!insurance_provider && insurance_provider !== "null" ? insurance_provider : null,
					policy_number: !!policy_number && policy_number !== "null" ? policy_number : null,
				...
				await db.MemberInsurance.update({
					member_id: fetchMember.id,
					insurance_provider: !!insurance_provider && insurance_provider !== "null" ? insurance_provider : findMemberInsurance.insurance_provider,
					policy_number: !!policy_number && policy_number !== "null" ? policy_number : findMemberInsurance.policy_number,
		...
		fetchMember = await db.Member.findOne({
			where: {
				id: fetchMember.id
			},
			include: [
...
exports.fetch_members_by_member_tokens = async (req, res, next) => {
	try {
		...
		let groupMembers = _.indexBy(fetchMembers, 'member_token')
		res.status(200).json({
			status: 'success',
...
exports.change_member_account = async (req, res, next) => {
	try {
		let {id} = req.params;
		...
		res.status(200).json({
			status: 'success',
			payload: null,
			message: 'Member account changed successfully'
		});

	} catch (error) {
		console.log("Error at changing member account method- PUT / :" + error);
		res.status(500).json({
			status: 'failed',
			payload: null,
			message: 'Error while changing member account'
		});
	}
};
